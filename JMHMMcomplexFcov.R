...
if (is.na(sim_num)){sim_num <- 4}
# if (is.na(sim_num)){sim_num <- 22}
...
Params2TranVectorTresid <- function(re_ind,fcovar_ind,len,params_tran,params_tran_fcovar){
  return(t(sapply(c(1:(len)),FUN = Params2Tran,params_tran = params_tran,re_ind=re_ind,params_tran_fcovar=params_tran_fcovar,fcovar_ind=fcovar_ind)))
}
...
Case4 <- function(act_obs,mu_act,mu_act_fcovar,sig_act,sig_act_fcovar,mu_light,mu_light_fcovar,sig_light,sig_light_fcovar,bivar_corr,light_LOD){
  
  mu_act_comb <- mu_act + mu_act_fcovar
  sig_act_comb <- sqrt(sig_act^2 + sig_act_fcovar^2)
  
  mu_light_comb <- mu_light + mu_light_fcovar
  sig_light_comb <- sqrt(sig_light^2 + sig_light_fcovar^2)
  
  mu_light_cond <- CalcCondMean(mu_light_comb,sig_light_comb,mu_act_comb,sig_act_comb,bivar_corr,act_obs)
  sig_light_cond <- CalcCondSig(sig_light_comb,bivar_corr)
  
  lognorm_dens <- dnorm(act_obs,mu_act_comb,sig_act_comb) * 
  ...
  for (i in 1:mix_num){
    for (j in 1:fcovar_num){
      lintegral_mat[i,j,1] <- log(integrate(Case4,lower = -Inf,upper = lod_act,
                                          emit_act[1,1,i],emit_act_fcovar[1,1,j],emit_act[1,2,i],emit_act_fcovar[1,2,j],
                                          emit_light[1,1,i],emit_light_fcovar[1,1,j],emit_light[1,2,i],emit_light_fcovar[1,2,j],
                                          corr_mat[i,1],lod_light)[[1]])
      
      lintegral_mat[i,j,2] <- log(integrate(Case4,lower = -Inf,upper = lod_act,
                                          emit_act[2,1,i],emit_act_fcovar[2,1,j],emit_act[2,2,i],emit_act_fcovar[2,2,j],
                                          emit_light[2,1,i],emit_light_fcovar[2,1,j],emit_light[2,2,i],emit_light_fcovar[2,2,j],
...
SimulateHMM <- function(day_length,num_of_people,init,params_tran_array,params_tran_array_fcovar,
  ...
  # fcovar_mix <- seq(3*fcovar_num,1,length.out=fcovar_num)
  
  mixture_mat <- t(rmultinom(num_of_people,1,pi_l_true))
  fcovar_mat <- t(rmultinom(num_of_people,1,rep(1/fcovar_num,fcovar_num)))
  # fcovar_mat <- t(rmultinom(num_of_people,1,fcovar_mix/sum(fcovar_mix)))
...
GradDesc <- function(params_tran_array,params_tran_array_fcovar,gradient,gradient_fcovar){
  print("GradDesc")
  dim1 <- prod(dim(params_tran_array))
  dim2 <- prod(dim(params_tran_array_fcovar))
  
  gradient_shaped <- array(0,dim(params_tran_array))
  gradient_fcovar_shaped <- array(0,dim(params_tran_array_fcovar))
  
  for(vcovar_ind in 1:vcovar_num){
    for(re_ind in 1:mix_num){
      gradient_shaped[re_ind,,vcovar_ind] <- c(gradient[1,,re_ind,vcovar_ind],gradient[2,,re_ind,vcovar_ind])
    }
    
    for(fcovar_ind in 1:fcovar_num){
      gradient_fcovar_shaped[fcovar_ind,,vcovar_ind] <- c(gradient_fcovar[1,,fcovar_ind,vcovar_ind],gradient_fcovar[2,,fcovar_ind,vcovar_ind])
    }
  }
  
  grad <- c(as.vector(gradient_shaped),as.vector(gradient_fcovar_shaped))
  
  params_tran_vec <- c(as.vector(params_tran_array),
                       as.vector(params_tran_array_fcovar))
  
  new_likelihood <- -Inf
  counter <- 1
  while (new_likelihood - likelihood < -1e5 ){
    
    step_size <- lm_step_size*iter_count*counter
    inf_mat <- -grad/step_size
    
    
    params_tran_vec_new <- params_tran_vec - inf_mat
    
    params_tran_array_new <- array(params_tran_vec_new[1:dim1], dim(params_tran_array))
    params_tran_array_fcovar_new <- array(params_tran_vec_new[(dim1+1):(dim1+dim2)], dim(params_tran_array_fcovar))
    
    tran_list <- GenTranList(params_tran_array_new,params_tran_array_fcovar_new,c(1:day_length),mix_num,fcovar_num,vcovar_num)
...
NR <- function(hessian_vec,cos_part_vec,sin_part_vec,cos_sin_part,
               hessian_vec_fcovar,cos_part_vec_fcovar,sin_part_vec_fcovar,cos_sin_part_fcovar,
               hessian_vec_both,cos_part_vec_both,sin_part_vec_both,cos_sin_part_both,
               params_tran_array,params_tran_array_fcovar,
               gradient,gradient_fcovar){
  ...
  for (init_state in 1:2){
    for(vcovar_ind in 1:vcovar_num){
      ...
      inf_mat <- SolveCatch(-hess,-grad)
      if(all(inf_mat == numeric(length(grad)))){
        print("Non Invertible Hessian")
        NR_count <<- NR_count + iter_count
        return(GradDesc(params_tran_array,params_tran_array_fcovar,gradient,gradient_fcovar))
      }
      
      if(any(abs(inf_mat)> 1) ){inf_mat <- inf_mat/max(abs(inf_mat))}
...
LM <- function(hessian_vec,cos_part_vec,sin_part_vec,cos_sin_part,
               hessian_vec_fcovar,cos_part_vec_fcovar,sin_part_vec_fcovar,cos_sin_part_fcovar,
               hessian_vec_both,cos_part_vec_both,sin_part_vec_both,cos_sin_part_both,
               params_tran_array,params_tran_array_fcovar,
               gradient,gradient_fcovar){
  ...
  hdim <- 3*mix_num+3*fcovar_num
  
  hess_array <- array(0,dim = c(hdim,hdim,2,vcovar_num))
  grad_array <- array(0,dim = c(hdim,2,vcovar_num))
  ...
  for (init_state in 1:2){
    for(vcovar_ind in 1:vcovar_num){
      ...
      hess_array[,,init_state,vcovar_ind] <- Symmetricize(hessian)
      grad_array[,init_state,vcovar_ind] <- c(as.vector(gradient[init_state,,,vcovar_ind]),as.vector(gradient_fcovar[init_state,,,vcovar_ind]))
    }
  }
  ...
  while (new_likelihood - likelihood < -1e-5 ){
    ...
    for (init_state in 1:2){
      for(vcovar_ind in 1:vcovar_num){
        ...
        # hess <- hess_array[,,init_state,vcovar_ind] + (diag(hdim)*lm_step_size)
        hess <- hess_array[,,init_state,vcovar_ind] + (diag(hess_array[,,init_state,vcovar_ind])*lm_step_size)
        grad <- grad_array[,init_state,vcovar_ind]
        ...
        inf_mat <- SolveCatch(hess,grad)
        if(all(inf_mat == numeric(length(grad))) | lm_step_size > 1e20){
          print("Non Invertible Hessian")
          lm_step_size <<- 1e8
          NR_count <<- NR_count + iter_count
          return(GradDesc(params_tran_array,params_tran_array_fcovar,gradient,gradient_fcovar))
    ...
    if (new_likelihood - likelihood < -1e-5 ){
      lm_step_size <<- lm_step_size * 10
    } else {lm_step_size <<- lm_step_size/10}
...
CalcTranC <- function(alpha,beta,act,light,params_tran_array,params_tran_array_fcovar,emit_act,emit_light,emit_act_fcovar,emit_light_fcovar,corr_mat,pi_l,lod_act,lod_light,lintegral_mat, fcovar_vec, vcovar_mat){
  ...
  return(LM(hessian_vec,cos_part_vec,sin_part_vec,cos_sin_part,
            hessian_vec_fcovar,cos_part_vec_fcovar,sin_part_vec_fcovar,cos_sin_part_fcovar,
            hessian_vec_both,cos_part_vec_both,sin_part_vec_both,cos_sin_part_both,
            params_tran_array,params_tran_array_fcovar,
            gradient,gradient_fcovar))
  
  # if( grad_desc){return(GradDesc(params_tran_array,params_tran_array_fcovar,gradient,gradient_fcovar))}
  # if(!grad_desc){return(NR(hessian_vec,cos_part_vec,sin_part_vec,cos_sin_part,
  #                                hessian_vec_fcovar,cos_part_vec_fcovar,sin_part_vec_fcovar,cos_sin_part_fcovar,
  #                                hessian_vec_both,cos_part_vec_both,sin_part_vec_both,cos_sin_part_both,
  #                                params_tran_array,params_tran_array_fcovar,
  #                                gradient,gradient_fcovar))}
...
EmitLogLike <- function(act,light,mu_act,mu_act_fcovar_vec,sig_act,sig_act_fcovar_vec,mu_light,mu_light_fcovar_vec,sig_light,sig_light_fcovar_vec,bivar_corr,lod_act,lod_light,weights_mat,fcovar_vec){
  
  log_like_vec <- numeric(prod(dim(act)))
  
  for (fcovar_ind in 1:fcovar_num){
    
    fcovar_vec_long <- rep(fcovar_vec, each = day_length)
    fcovar_ind_vec <- fcovar_vec_long==fcovar_ind
    
    lintegral <- log(integrate(Case4,lower = -Inf,upper = lod_act,
                               mu_act,mu_act_fcovar_vec[fcovar_ind],
                               sig_act,sig_act_fcovar_vec[fcovar_ind],
                               mu_light,mu_light_fcovar_vec[fcovar_ind],
                               sig_light,sig_light_fcovar_vec[fcovar_ind],
                               bivar_corr,lod_light)[[1]])
    
    log_like <- logClassificationC(act[fcovar_ind_vec],light[fcovar_ind_vec],
                                   mu_act,mu_act_fcovar_vec[fcovar_ind],
                                   sig_act,sig_act_fcovar_vec[fcovar_ind],
                                   mu_light,mu_light_fcovar_vec[fcovar_ind],
                                   sig_light,sig_light_fcovar_vec[fcovar_ind],
    ...
    log_like_vec[fcovar_ind_vec] <- log_like
    
    # log_like_vec <- log_like_vec + (log_like*(fcovar_vec_long==fcovar_ind))
    
  }
  
  
  return(-sum(log_like_vec * weights_mat))
}


EmitLogLikeFcovar <- function(act,light,mu_act_vec,mu_act_fcovar,sig_act_vec,sig_act_fcovar,mu_light_vec,mu_light_fcovar,sig_light_vec,
                              sig_light_fcovar,bivar_corr_vec,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec){
  ...
  for (re_ind in 1:mix_num){
    bivar_corr <- bivar_corr_vec[re_ind]
    
    weights_vec <- as.vector(weights_array[,,re_ind])
    
    lintegral <- log(integrate(Case4,lower = -Inf,upper = lod_act,
                               mu_act_vec[re_ind],mu_act_fcovar,
                               sig_act_vec[re_ind],sig_act_fcovar,
                               mu_light_vec[re_ind],mu_light_fcovar,
                               sig_light_vec[re_ind],sig_light_fcovar,
                               bivar_corr,lod_light)[[1]])
    ...
    log_like_sum <- log_like_sum + sum(log_like * weights_vec[fcovar_ind_vec])
    
    
  }
  
  return(-sum(log_like_sum))
}

CalcActMean <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec){
  ...
  mu_act <- optimize(EmitLogLike, c(-20,10), act = act, light = light, 
                     ...
                     mu_act_fcovar_vec = emit_act_fcovar[mc_state,1,],
                     sig_act_fcovar_vec = emit_act_fcovar[mc_state,2,],
                     mu_light_fcovar_vec = emit_light_fcovar[mc_state,1,],
                     sig_light_fcovar_vec = emit_light_fcovar[mc_state,2,],
...
CalcActSig <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec){
...
# CalcLightMean(2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_sleep,2,fcovar_vec)
# 
...
CalcLightMean <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec){
...
CalcLightSig <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec){
...
CalcActMeanFcovar <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec){
  
  mu_act_fcovar <- optimize(EmitLogLikeFcovar, c(-20,10), act = act, light = light, 
                            mu_act_vec = emit_act[mc_state,1,],
                            sig_act_vec = emit_act[mc_state,2,],
                            mu_light_vec = emit_light[mc_state,1,], 
                            sig_light_vec = emit_light[mc_state,2,],
                            # mu_act_fcovar = emit_act_fcovar[mc_state,1,fcovar_ind],
                            sig_act_fcovar = emit_act_fcovar[mc_state,2,fcovar_ind],
                            mu_light_fcovar = emit_light_fcovar[mc_state,1,fcovar_ind],
                            sig_light_fcovar = emit_light_fcovar[mc_state,2,fcovar_ind],
                            bivar_corr_vec = corr_mat[,mc_state],
                            lod_act = lod_act, lod_light = lod_light,
                            weights_array = weights_array,
                            fcovar_ind,fcovar_vec)$minimum
  return(mu_act_fcovar)
}

CalcLightMeanFcovar <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec){
...
CalcActSigFcovar <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec){
  
  mu_act_fcovar <- optimize(EmitLogLikeFcovar, c(0,10), act = act, light = light, 
                            mu_act_vec = emit_act[mc_state,1,],
                            sig_act_vec = emit_act[mc_state,2,],
                            mu_light_vec = emit_light[mc_state,1,], 
                            sig_light_vec = emit_light[mc_state,2,],
...
CalcLightSigFcovar <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec){
...
CalcBivarCorr <- function(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec){
...
UpdateNorm <- function(FUN,mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep){
  opt_param_vec <- numeric(mix_num)
  ...
  for (re_ind in 1:dim(emit_act)[3]){
    opt_param_vec[re_ind] <- FUN(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,re_ind,fcovar_vec)
  }
...
UpdateNormFcovar <- function(FUN,mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep){
  opt_param_vec <- numeric(fcovar_num)
  
  if(mc_state == 1){weights_array <- weights_array_wake}
  if(mc_state == 2){weights_array <- weights_array_sleep}
  
  for (fcovar_ind in 1:dim(emit_act_fcovar)[3]){
    opt_param_vec[fcovar_ind] <- FUN(mc_state,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array,fcovar_ind,fcovar_vec)
  }
...
SolveLM <- function(hess,grad,step_size) {
  tryCatch(
    {
      hess <- hess + (diag(dim(hess)[1]) * step_size)
      solve(hess,grad,tol=1e-50)
    },
    error = function(cond) {
...
CalcBIC <- function(new_likelihood,mix_num,act,light){
  ...
  num_of_param <- mix_num + 
    mix_num*6*2 + fcovar_num*6*2 + 
    mix_num*4 + fcovar_num*4 + mix_num*2 + 
    mix_num-1 + 
    mix_num-1 + fcovar_num-1
  
  bic <- num_of_param * log(sum(!is.na(act)) + sum(!is.na(light))) - (2 * new_likelihood)
  return(bic)
}

CalcBLHaz <- function(beta_mat, re_prob,surv_event,surv_time,fcovar_mat){
  n <- length(surv_event)
  bline_vec <- numeric(n)
...
CalcBetaHelper <- function(beta_vec_long){
  ...
  if (mix_num > 2){
    temp <- diag(hessian[1:(mix_num-1),1:(mix_num-1)])
    hessian[1:(mix_num-1),1:(mix_num-1)] <- 0
    diag(hessian[1:(mix_num-1),1:(mix_num-1)]) <- temp
  } 
    
  temp <- diag(hessian[mix_num:dim(hessian)[1],mix_num:dim(hessian)[2]])
  hessian[mix_num:dim(hessian)[1],mix_num:dim(hessian)[2]] <- 0
  diag(hessian[mix_num:dim(hessian)[1],mix_num:dim(hessian)[2]]) <- temp
  
  nr_fact <- 0
  step_size <- .01
  while(all(nr_fact == 0 )){
    nr_fact <- SolveLM(-hessian,-gradient,step_size)
    step_size <- step_size * 10
  }
  
  
  if(any(abs(nr_fact)> 1) ){nr_fact <- nr_fact/max(abs(nr_fact))}
  nr_fact <- c(0,nr_fact)
...
TranLogLike <- function(params_tran_vec){
  
  loglike <- 0
  dim1 <- prod(params_tran_array_dim)
  dim2 <- prod(params_tran_array_fcovar_dim)
  
  params_tran_array <- array(params_tran_vec[1:dim1], params_tran_array_dim)
  params_tran_array_fcovar <- array(params_tran_vec[(dim1+1):(dim1+dim2)], params_tran_array_fcovar_dim)
    ...
    for (new_state in 1:2){
      ...
      for (ind in 1:length(alpha)){
        ...
        for(re_ind in 1:mix_num){
          ...
          loglike <- loglike + sum(tran_vals_re_array[init_state,new_state,,ind,re_ind] * log(tran_vec))
...
ParamsArray2DF <- function(params_tran_array,params_tran_array_fcovar){
  ...
  for (re_ind in 1:mix_num){
    for (vcovar_ind in 1:vcovar_num){
      for(fcovar_ind in 1:fcovar_num){
        tran_mat <- Params2TranVectorTresid(re_ind,fcovar_ind,96,params_tran_array[,,vcovar_ind],params_tran_array_fcovar[,,vcovar_ind])
        tosleep <- tran_mat[,3]
        towake <- tran_mat[,2]
...
Emit2DF <- function(emit_act,emit_light,emit_act_fcovar,emit_light_fcovar){
  
  emit_df <- data.frame(prob = c(),
                        par  = c(),
                        state = c(),
                        type = c(),
                        age = c(),
  ...
  for (re_ind in 1:mix_num){
    for(fcovar_ind in 1:fcovar_num){
      for(mc_state in 1:2){
        ...
        act_mean <- emit_act[mc_state,1,re_ind] + emit_act_fcovar[mc_state,1,fcovar_ind]
        act_sig <- sqrt(emit_act[mc_state,2,re_ind]^2 + emit_act_fcovar[mc_state,2,fcovar_ind]^2)
        
        light_mean <- emit_light[mc_state,1,re_ind] + emit_light_fcovar[mc_state,1,fcovar_ind]
        light_sig <- sqrt(emit_light[mc_state,2,re_ind]^2 + emit_light_fcovar[mc_state,2,fcovar_ind]^2)
        
        emit_df_working <- data.frame(prob = c(act_mean,act_sig,light_mean,light_sig),
                                      par  = c("Mean","SE","Mean","SE"),
                                      state = rep(mc_state,4),
                                      type = c("Activity","Activity","Light","Light"),
                                      age = fcovar_ind,
                                      mixture = re_ind)
        emit_df <- rbind(emit_df,emit_df_working)
      }
...
params_tran_week_fcovar_true <- matrix(rep(c(-1.5,2,.3,-2,-2,-1),fcovar_num),ncol = 6,byrow = T) 
params_tran_weekend_fcovar_true <- matrix(rep(c(-2,1.5,1,-.75,-1,-.5),fcovar_num),ncol = 6,byrow = T) 
...
params_tran_array_fcovar_true[,,1] <- params_tran_week_fcovar_true 
params_tran_array_fcovar_true[,,2] <- params_tran_weekend_fcovar_true 
...
emit_act_fcovar_true[1,1,] <- seq(3,0,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_act_fcovar_true[1,2,] <- seq(2,0,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_act_fcovar_true[2,1,] <- seq(0,2,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_act_fcovar_true[2,2,] <- seq(0,1,length.out = fcovar_num^2)[c(1:fcovar_num)^2]

emit_light_fcovar_true <- array(0, c(2,2,fcovar_num))
emit_light_fcovar_true[1,1,] <- seq(4,0,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_light_fcovar_true[1,2,] <- seq(1,0,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_light_fcovar_true[2,1,] <- seq(0,3,length.out = fcovar_num^2)[c(1:fcovar_num)^2]
emit_light_fcovar_true[2,2,] <- seq(0,.5,length.out = fcovar_num^2)[c(1:fcovar_num)^2]

corr_mat_true <- matrix(0,ncol = 2,nrow = mix_num)
corr_mat_true[,1] <- seq(.8,-.6,length.out = mix_num)
corr_mat_true[,2] <- seq(.6,-.8,length.out = mix_num)
...
bhaz_vec <- CalcBLHaz(beta_mat,re_prob,surv_event,surv_time,fcovar_mat)
...
iter_count <- 1
beta_bool <- F
params_tran_array_old <- params_tran_array + 1
while(abs(like_diff) > 1e-3){
  ...
  params_tran_array_list <- CalcTranC(alpha,beta,act,light,params_tran_array,params_tran_array_fcovar,
                                      emit_act,emit_light,emit_act_fcovar,emit_light_fcovar,corr_mat,
                                      pi_l,lod_act,lod_light,lintegral_mat,fcovar_vec,vcovar_mat)
  ...
  # thess <- hessian(TranGrad, params_tran_vec)
  # 
  # params_tran_vec <- params_tran_vec - solve(-thess,-tgrad)
  # 
  # # params_tran_vec <- c(as.vector(params_tran_array),as.vector(params_tran_array_fcovar))
  # # tran_nlm <- nlm(TranLogLike,params_tran_vec,iterlim = 1,stepmax = 1)
  # # params_tran_vec <- tran_nlm$estimate
  # 
  # params_tran_array <- array(params_tran_vec[1:dim1], params_tran_array_dim)
  # params_tran_array_fcovar <- array(params_tran_vec[(dim1+1):(dim1+dim2)], params_tran_array_fcovar_dim)
  ...
  corr_mat[,1] <- UpdateNorm(CalcBivarCorr,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  corr_mat[,2] <- UpdateNorm(CalcBivarCorr,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_act[1,1,] <- UpdateNorm(CalcActMean,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_act[2,1,] <- UpdateNorm(CalcActMean,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_light[1,1,] <- UpdateNorm(CalcLightMean,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_light[2,1,] <- UpdateNorm(CalcLightMean,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_act[1,2,] <- UpdateNorm(CalcActSig,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_act[2,2,] <- UpdateNorm(CalcActSig,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_light[1,2,] <- UpdateNorm(CalcLightSig,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_light[2,2,] <- UpdateNorm(CalcLightSig,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  ##### Fcovar Normal Param

  emit_act_fcovar[1,1,] <- UpdateNormFcovar(CalcActMeanFcovar,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_act_fcovar[2,1,] <- UpdateNormFcovar(CalcActMeanFcovar,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_light_fcovar[1,1,] <- UpdateNormFcovar(CalcLightMeanFcovar,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_light_fcovar[2,1,] <- UpdateNormFcovar(CalcLightMeanFcovar,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_act_fcovar[1,2,] <- UpdateNormFcovar(CalcActSigFcovar,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_act_fcovar[2,2,] <- UpdateNormFcovar(CalcActSigFcovar,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)

  emit_light_fcovar[1,2,] <- UpdateNormFcovar(CalcLightSigFcovar,1,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  emit_light_fcovar[2,2,] <- UpdateNormFcovar(CalcLightSigFcovar,2,act,light,emit_act,emit_act_fcovar,emit_light,emit_light_fcovar,corr_mat,lod_act,lod_light,weights_array_wake,weights_array_sleep)
  ...
  if(like_diff < 0){
    ...
    params_tran_array_fcovar <- params_tran_array_fcovar_old
  ...
  params_tran_array_fcovar_old <- params_tran_array_fcovar
  
  if (iter_count %% 20 == 0){
    
    true_params <- list(init_true,params_tran_array_true,params_tran_array_fcovar_true,
                        emit_act_true,emit_light_true,
                        emit_act_fcovar_true,emit_light_fcovar_true,
                        corr_mat_true,pi_l_true,
                        beta_mat_true)
    
    est_params <- list(init,params_tran_array,params_tran_array_fcovar,
                       emit_act,emit_light,
                       emit_act_fcovar,emit_light_fcovar,
                       corr_mat,pi_l,re_prob,
                       beta_mat,bhaz_vec)
...
tran_df <- ParamsArray2DF(params_tran_array,params_tran_array_fcovar)
emit_df <- Emit2DF(emit_act,emit_light,emit_act_fcovar,emit_light_fcovar)

if (!real_data){
  tran_df_true <- ParamsArray2DF(params_tran_array_true,params_tran_array_fcovar_true)
  emit_df_true <- Emit2DF(emit_act_true,emit_light_true,emit_act_fcovar_true,emit_light_fcovar_true)
  
  tran_df <- tran_df %>% mutate(truth = tran_df_true[,1])
  tran_df <- tran_df %>% mutate(resid = prob - truth)
  
  emit_df <- emit_df %>% mutate(truth = emit_df_true[,1])
  emit_df <- emit_df %>% mutate(resid = prob - truth)
}




if( real_data){mix_ident <- mixture_mat}
if(!real_data){mix_ident <- re_prob}
bhaz_vec_true <- CalcBLHaz(beta_mat_true,mix_ident,surv_event,surv_time,fcovar_mat)

true_params <- list(init_true,params_tran_array_true,params_tran_array_fcovar_true,
                    emit_act_true,emit_light_true,
                    emit_act_fcovar_true,emit_light_fcovar_true,
                    corr_mat_true,pi_l_true,mix_ident,
                    beta_mat_true,bhaz_vec_true)

est_params <- list(init,params_tran_array,params_tran_array_fcovar,
                   emit_act,emit_light,
                   emit_act_fcovar,emit_light_fcovar,
                   corr_mat,pi_l,mix_ident,
                   beta_mat,bhaz_vec,
                   tran_df,emit_df)

# init <- to_save[[2]][[1]]
# params_tran_array <- to_save[[2]][[2]]
# params_tran_array_fcovar <- to_save[[2]][[3]]
# emit_act <- to_save[[2]][[4]]
# emit_light <- to_save[[2]][[5]]
# emit_act_fcovar <- to_save[[2]][[6]]
# emit_light_fcovar <- to_save[[2]][[7]]
# corr_mat <- to_save[[2]][[8]]
# pi_l <- to_save[[2]][[9]]
# beta_mat <- to_save[[2]][[10]]
# bhaz_vec <- to_save[[2]][[11]]
# bline_vec <- bhaz_vec[[1]]
# cbline_vec <- bhaz_vec[[2]]
# tran_list <- GenTranList(params_tran_array,params_tran_array_fcovar,c(1:day_length),mix_num,fcovar_num,vcovar_num)
# lintegral_mat <- CalcLintegralMat(emit_act,emit_light,emit_act_fcovar,emit_light_fcovar,corr_mat,lod_act,lod_light)
